...
export default function ShareHeroMany() {
    ...
    async function fetchData() {
        try {
            const response = await axios.get(`/brain/selected/${shareID}`);
    ...
    return (
        <div className="bg-background min-h-screen h-auto w-full ml-0 xs:ml-[96px] lg:ml-[300px]">
            <div className="xxs:p-10 p-5">
                ...
                {
                    error && <div className="text-primaryBtn-hover h-full text-2xl mt-5 stroke-[5px] px-20 pt-10 flex flex-col justify-start items-start">
                        <h1 className="flex gap-4">
                            Link is Invalid or Expired
                            <Button onClick={() => {
                                router.push("/dashboard")
                            }} variant="primary" size="sm" text="go back" startIcon={<BackIcon size="sm" />} />
                        </h1>
                    </div>
                }
            </div>
