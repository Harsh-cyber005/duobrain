...
export default function SearchPage() {
	const {setModalOpen, setModalComponent, setShare, setAuthName, authName, setShareLink, baseShareLink, setSidebarOpen, setToken, selected, selectActive, setSelectActive, setSelected, shareMany, setShareMany, setShareManyLink, baseShareManyLink, setInitial, setFromColor, setFilter, setSearchedData, searchedData, setTotalContentShare } = useAppContext();
	const router = useRouter();
	const [h1Display, setH1Display] = useState("Loading...");
	const [loading, setLoading] = useState(true);
    const [search, setSearch] = useState("");
    const [filterr, setFilterr] = useState<"youtube" | "instagram" | "pinterest" | "links" | "tweets" | "document" | "all" | "">("");
    const params = useParams();

    useEffect(()=>{
        const query = (params.query as string).replaceAll("%26","&").replaceAll("%3D","=").replaceAll("%2B","+").replaceAll("%20"," ");
        const s = query.split("&")[0].split("=")[1].replaceAll("+"," ");
        const f = query.split("&")[1].split("=")[1].replaceAll("+"," ");
        setSearch(s);
		setFilterr(f as "youtube" | "instagram" | "pinterest" | "links" | "tweets" | "document" | "all" | "");
        handleSearch(s,f);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    },[params])

	async function handleSearch(s: string, f: string) {
            setLoading(true);
            try {
                const response = await axios.post('/search/embedded', {
                    "query": s,
                    "filter": f
                }, {
                    headers: {
	...
	useEffect(() => {
		if (searchedData.length === 0) {
			...
			setTotalContentShare(searchedData.length);
			setH1Display(authName);
		}
	// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [authName, searchedData])
	...
	async function deleteCard(id: string) {
		try {
			...
			setSearchedData(searchedData.filter((item) => item._id !== id));
		} catch (error) {
			console.log(error);
		}
	}
	...
	async function deleteSelected(): Promise<number> {
		try {
			...
			setSearchedData(searchedData.filter((item) => !selected.includes(item._id)));
			return 1;
		} catch (error) {
			console.log(error);
			return 0;
	...
	return (
		<div className=" min-h-screen h-auto w-full ml-0 xs:ml-[96px] lg:ml-[300px]">
			<div className="xxs:p-10 p-5 relative">
